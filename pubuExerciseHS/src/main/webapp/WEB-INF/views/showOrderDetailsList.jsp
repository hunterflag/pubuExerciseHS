	...
	<c:otherwise>
		...
		<c:if test="${oddts.size()>0}">
			<c:forEach items="${oddts}" var="oddt" >
				<TR>
				<TD>${oddt.oddt_id}
				<TD>${oddt.odBean.od_id}
				<TD>${oddt.odBean.ctmBean.ctm_account}
				<TD>${oddt.pdtBean.pd_name}
				<TD>${oddt.pdtBean.pd_price}
				<TD>${oddt.oddt_price}
				<TD>${oddt.oddt_number}
				<TD>${oddt.oddt_price * oddt.oddt_number}
			</c:forEach>
		</c:if>
		<TR><TH colspan=4>總價 <TD colspan=4> ${oddts[0].odBean.od_total_price}
		</table>
	</c:otherwise>
</c:choose>
</body>
